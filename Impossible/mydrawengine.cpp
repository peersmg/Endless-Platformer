...
// Gives a string to describe a problem based on an HRESULT
...
ErrorType MyDrawEngine::Start(HWND hwnd, bool bFullScreen)
{
	...
	instance->AddFont(L"Eras Bold ITC", 72, false, false);
  instance->AddFont(L"Eras Bold ITC", 124, false, false);
...
// Set the presentation parameters options
    ...
    SetWindowPos(m_Hwnd, HWND_NOTOPMOST,
		GetSystemMetrics(SM_CXSCREEN) / 2-(400), GetSystemMetrics(SM_CYSCREEN)/2-(225),
                     800,//m_ScreenWidth,
                     450,//m_ScreenHeight,
                     SWP_SHOWWINDOW);
...
ErrorType MyDrawEngine::Flip()
{
  ...
  //ADDED TO STOP BLACK SCREEN WHEN ALT TABBING
...
ErrorType MyDrawEngine::WriteText(int x, int y, const wchar_t text[], int colour,FontIndex fontIndex 
  , Alignment alignment,double rotation, Vector2D scale)
{
  ...
  D3DXMATRIX mat;
  D3DXVECTOR2 scaling(scale.XValue, scale.YValue);
  D3DXVECTOR2 spriteCentre = D3DXVECTOR2(rect.left+(width/2), rect.top + (height/2));
  ...
  // I REPLACED NULL WITH sprite
  err = fit->second.m_pFont->DrawText(m_lpSprite, text, -1, &rect, 0, colour);
...
ErrorType MyDrawEngine::WriteText(Vector2D position, const wchar_t text[], int colour, FontIndex fontIndex
  , Alignment alignment,double rotation, Vector2D scale)
{
  return WriteText(int(position.XValue), int(position.YValue), text, colour, fontIndex, alignment, rotation, scale);
}
