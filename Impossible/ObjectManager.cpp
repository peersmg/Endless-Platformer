...
{
  auto it = m_allObjectList.begin();
  auto itB = m_allObjectList.begin();
  auto itU = m_allObjectList.begin();

  //Added code here to ensure backgrounds are always drawn at the back
  for (; itB != m_allObjectList.end(); ++itB)
  {
    if ((*itB)->IsActive() && (*itB)->GetSceneNumber() == m_currentScene 
      && (*itB)->GetType() == BACKGROUND)
    {
      (*itB)->Draw();
    }
  }
  ...
  //Added code here to ensure the UI is always drawn at the front
  for (; itU != m_allObjectList.end(); ++itU)
  {
    if ((*itU)->IsActive() && (*itU)->GetSceneNumber() == m_currentScene
      && ((*itU)->GetType() == UI || (*itU)->GetType() == GM))
    {
      (*itU)->Draw();
    }
  }
