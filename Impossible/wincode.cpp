...
// Version 13.03
// Game is windowed by default
// Version 13.01   4/3/2015
// Fixed game loop
...
// Version 12.10 6/12/12
// Added keyboard buffer logging for event WM_CHAR

// wincode.cpp
// Creates a window that fills the primary monitor
...
KeyboardBuffer g_KeyboardBuffer;
HWND g_hWnd = NULL;
	...
	case WM_CREATE:				// If window has been created
		{
			...
			ShowCursor(TRUE);				// Do any initialisation
				   ...
				   int nCmdShow)				// Integer that tells how the window is to appear. Active/maximized, etc
{
	...
	if(CreateDXWindow(L"GAME NAME", 800, 600) == SUCCESS)
	{
		// The event loop
...
ErrorType CreateDXWindow(wchar_t* title, int width, int height)
{
	...
	WindowRect.left = GetSystemMetrics(SM_CXSCREEN) / 2;
	WindowRect.right=width;
	WindowRect.top = GetSystemMetrics(SM_CYSCREEN) / 2;
